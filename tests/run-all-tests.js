/**
 * Script per eseguire tutti i test della dashboard
...
async function runAllTests() {
  console.log(`${colors.bright}${colors.magenta}🚀 AVVIO SUITE COMPLETA DI TEST - DASHBOARD TEAM${colors.reset}`);
  ...
  const tests = [
    {
      file: path.join(__dirname, 'backend', 'test-dashboard-connectivity.js'),
      name: 'Test Backend - Connettività Dashboard'
    },
    {
      file: path.join(__dirname, 'backend', 'test-dashboard-authenticated.js'),
      name: 'Test Backend - Dashboard con Autenticazione'
    },
    {
      file: path.join(__dirname, 'backend', 'test-dashboard-team-api.js'),
      name: 'Test Backend - API Dashboard Team'
    },
    {
      file: path.join(__dirname, 'backend', 'test-dashboard-player-api.js'),
      name: 'Test Backend - API Dashboard Player'
    },
    {
      file: path.join(__dirname, 'integration', 'test-dashboard-complete.js'),
      name: 'Test Integrazione - Dashboard Completa'
    },
    {
      file: path.join(__dirname, 'frontend', 'components', 'test-team-dashboard.js'),
      name: 'Test Frontend - TeamDashboard Component'
    },
    {
      file: path.join(__dirname, 'frontend', 'components', 'test-filters.js'),
      name: 'Test Frontend - Filters Component'
    },
    {
      file: path.join(__dirname, 'frontend', 'integration', 'test-dashboard-integration.js'),
  ...
  if (passed === total) {
    console.log(`\n${colors.bright}${colors.green}🎉 TUTTI I TEST SONO PASSATI! 🎉${colors.reset}`);
    console.log(`${colors.green}La dashboard funziona correttamente sia per la vista Squadra che Giocatore.${colors.reset}`);
  } else {
    console.log(`\n${colors.bright}${colors.red}⚠️  ALCUNI TEST SONO FALLITI! ⚠️${colors.reset}`);
