-- ===============================================================
-- 🧮 FISCAL SETUP V2 - MIGRAZIONE SUPABASE (SICURA)
-- Script per creare nuove tabelle parlanti e rimuovere legacy
-- VERSIONE SICURA: Controlla esistenza tabelle prima di migrare
-- ===============================================================
...
-- ===============================================================
-- 3️⃣ MIGRAZIONE DATI ESISTENTI (SICURA)
-- ===============================================================

-- Controlla se la tabella tax_rates esiste prima di migrare
DO $$
BEGIN
    -- Migra dati da TaxRate legacy a tax_rates_v2 (solo se la tabella esiste)
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'soccerxpro' AND table_name = 'tax_rates') THEN
        RAISE NOTICE '📊 Trovata tabella tax_rates legacy, avvio migrazione...';
        ...
        RAISE NOTICE '✅ Migrazione dati da tax_rates completata';
    ELSE
