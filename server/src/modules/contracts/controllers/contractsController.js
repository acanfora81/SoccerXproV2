// Percorso: server/src/modules/contracts/controllers/contractsController.js
...
const createContract = async (req, res) => {
  try {
    ...
    // Auto-genera contractNumber se non fornito
    let finalContractNumber = contractNumber && typeof contractNumber === 'string' && contractNumber.trim() !== '' ? contractNumber.trim() : null;
    
    if (!finalContractNumber) {
      // Trova il numero più alto esistente per il team e incrementa
      const lastContract = await prisma.contracts.findFirst({
        where: { teamId, contractNumber: { not: null } },
        orderBy: { contractNumber: 'desc' },
        select: { contractNumber: true }
      });
      
      if (lastContract && lastContract.contractNumber) {
        // Estrai il numero dal formato (es. "CONT-2024-001" -> 1)
        const match = lastContract.contractNumber.match(/(\d+)$/);
        const nextNum = match ? parseInt(match[1]) + 1 : 1;
        finalContractNumber = `CONT-${new Date().getFullYear()}-${String(nextNum).padStart(3, '0')}`;
      } else {
        // Primo contratto del team
        finalContractNumber = `CONT-${new Date().getFullYear()}-001`;
      }
    }
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const contract = await tx.contracts.create({
        data: {
          ...
          imageRights: (imageRights !== undefined && imageRights !== null && imageRights !== '' && imageRights !== 0) ? parseFloat(parseItalianNumber(imageRights)) : (imageRights === 0 ? 0 : null),
          loyaltyBonus: (loyaltyBonus !== undefined && loyaltyBonus !== null && loyaltyBonus !== '' && loyaltyBonus !== 0) ? parseFloat(parseItalianNumber(loyaltyBonus)) : (loyaltyBonus === 0 ? 0 : null),
          signingBonus: (signingBonus !== undefined && signingBonus !== null && signingBonus !== '' && signingBonus !== 0) ? parseFloat(parseItalianNumber(signingBonus)) : (signingBonus === 0 ? 0 : null),
          accommodationBonus: (accommodationBonus !== undefined && accommodationBonus !== null && accommodationBonus !== '' && accommodationBonus !== 0) ? parseFloat(parseItalianNumber(accommodationBonus)) : (accommodationBonus === 0 ? 0 : null),
          carAllowance: (carAllowance !== undefined && carAllowance !== null && carAllowance !== '' && carAllowance !== 0) ? parseFloat(parseItalianNumber(carAllowance)) : (carAllowance === 0 ? 0 : null),
      ...
      console.log('🔵 Contratto creato con ID:', contract.id);

      // Recupera il contratto completo con relazioni DENTRO la transazione
      try {
        ...
        console.log('🔵 Contratto recuperato con successo:', fullContract ? 'OK' : 'NULL');
        return fullContract;
      } catch (fetchError) {
        console.error('❌ Errore nel recupero contratto:', fetchError);
        // Ritorna solo il contratto base senza relazioni se il recupero fallisce
        return contract;
      }
    });

    console.log('🔵 Transazione completata, invio risposta...');
