...
const RegionalStep = () => {
  ...
  const [brackets, setBrackets] = useState([
    { min: 0, max: 28000, rate: 0.73 },
    { min: 28001, max: 55000, rate: 1.33 },
    { min: 55001, max: null, rate: 1.73 }
  ]);
  ...
  const handleSave = async () => {
    if (!region) {
      setMessage({ type: 'error', text: 'Specifica una regione nel contesto fiscale' });
      return;
    }
    ...
    try {
      ...
      await axios.post(
        `${API_BASE_URL}/api/fiscal-setup/step/regional`,
        {
          teamId,
          year,
          region,
  ...
  return (
    <Card>
      <CardHeader>
        <h3 className="text-lg font-semibold">Addizionale Regionale</h3>
      </CardHeader>
      <CardContent>
        <div className="space-y-4">
          <p className="text-sm text-gray-600">
            Configura l'addizionale regionale IRPEF. Può essere flat (aliquota fissa) o progressiva.
          </p>
          ...
          <div className="flex justify-end">
            ...
            >
              ...
              {saving ? 'Salvataggio...' : 'Salva Regionale'}
            </button>
          </div>
        </div>
      </CardContent>
